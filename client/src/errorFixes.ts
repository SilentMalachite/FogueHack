...
export class GameErrorFixer {
  ...
  static fixQuestSystemInitialization() {
    ...
    try {
      ...
      if (!Array.isArray(availableQuests) || !Array.isArray(activeQuests) || !Array.isArray(completedQuests)) {
  ...
  static fixUseGameStateHook() {
    ...
    try {
      ...
      const requiredMethods = [
        'startNewGame', 'movePlayer', 'castSpell', 'craftItem',
        'getAvailableQuests', 'getActiveQuests', 'getCompletedQuests', 'acceptQuest',
        'saveGame', 'loadGame'
      ];
      
      const missingMethods = requiredMethods.filter(method => typeof (gameState as any)[method] !== 'function');
