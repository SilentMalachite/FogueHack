...
export class SpellSystem {
  ...
  castSpell(
    ...
    // 距離チェック
    const distance = Math.abs(caster.position.x - target.x) + Math.abs(caster.position.y - target.y);
      ...
      switch (effect.type) {
        case 'heal':
          if (effect.target === 'self') {
            const healAmount = effect.value === 9999 ? caster.maxHp : Math.min(effect.value, caster.maxHp - caster.hp);
  ...
  private damageAreaTargets(center: Position, damage: number, gameState: GameState): void {
    ...
    gameState.monsters.forEach(monster => {
      const distance = Math.abs(monster.position.x - center.x) + Math.abs(monster.position.y - center.y);
      if (distance <= radius) {
