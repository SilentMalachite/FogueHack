...
export class GameTestRunner {
  ...
  testSystemInitialization(): TestResult {
    try {
      ...
      return { name: 'システム初期化', passed: true, details: 'ゲームエンジン、プレイヤー、ダンジョン正常' };
    } catch (error) {
  ...
  testMagicSystem(): TestResult {
    try {
      ...
      return { name: '魔法システム', passed: true, details: `${availableSpells.length}個の魔法が利用可能、ヒール効果確認済み` };
    } catch (error) {
  ...
  testQuestSystem(): TestResult {
    try {
      ...
      return { name: 'クエストシステム', passed: true, details: `${availableQuests.length}個のクエスト利用可能、受諾機能正常` };
    } catch (error) {
  ...
  testCraftingSystem(): TestResult {
    try {
      ...
      return { name: 'アイテム合成システム', passed: true, details: `${recipes.length}個のレシピ利用可能、合成判定機能正常` };
    } catch (error) {
  ...
  testCombatSystem(): TestResult {
    try {
      ...
      return { name: '戦闘システム', passed: true, details: `${gameState.monsters.length}体のモンスター生成、戦闘システム動作中` };
    } catch (error) {
