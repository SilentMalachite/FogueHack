...
const GameMap: React.FC = () => {
      ...
      <div key={y} className="flex leading-none" style={{ height: "1em" }}>
        {row.map((tile, x) => {
          ...
          if (player.position.x === x && player.position.y === y) {
            return (
              <span key={x} className="text-white font-bold" style={{ width: "1ch" }}>
                @
              </span>
            );
          }
          ...
          if (monster) {
            return (
              <span key={x} style={{ color: monster.color, width: "1ch" }} className="font-bold">
                {monster.symbol}
              </span>
            );
          }
          ...
          if (hasItem) {
            return (
              <span key={x} className="text-yellow-400 font-bold" style={{ width: "1ch" }}>
                *
              </span>
            );
          }
  ...
  return <div className="font-mono text-sm leading-none overflow-hidden">{renderMap()}</div>;
};
